...
class Experiment:
    ...
    def __init__(self, learning_rate, model, epochs, batch_size, max_length, neg, dataset_name, input_dim = 768, cons_list_sin = ['log', 'exp', 'cos', 'Integer', 'sin', 'Symbol'], cons_list_dou = ['Mul', 'Add', 'Pow']):
        ...
        if self.model_name == 'rgcn':
            self.model = GraphLatentReasoning_RGCN(model, 2, self.device)
    ...
    def construct_graph(self, examples, var):
        ...
        examples = {"node_list": node_list, "edge_index": edge_index, "edge_type": edge_type, "var_idx": var_idx}
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, default="rgcn", nargs="?",
                    help="Which model to use, options: rgcn")
